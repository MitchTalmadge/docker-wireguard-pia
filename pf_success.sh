...
if [ $FIREWALL -eq 1 ]; then
  iptables -A INPUT -p tcp -i wg1 --dport "$1" -j ACCEPT
  iptables -A INPUT -p udp -i wg1 --dport "$1" -j ACCEPT
...
if [ -n "$PF_DEST_IP" ] && [ -n "$FWD_IFACE" ]; then
  ...
  iptables -A FORWARD -i wg1 -o "$FWD_IFACE" -p tcp -d "$PF_DEST_IP" --dport "$1" -j ACCEPT
  iptables -A FORWARD -i wg1 -o "$FWD_IFACE" -p udp -d "$PF_DEST_IP" --dport "$1" -j ACCEPT
